...
def sendData(accident_info):
    ...
    files = {
        'image': ('accidentImg.png', open(accident_info['imagePath'], 'rb'), 'image/png'),
                ...
                if accident == 1: # 사고발생하면
                    ...
                    if(accident_count == 5):
                        ...
                        accident_info = {
                            ...
                            "accuracy": f"{confidence.item():.2f}",
                            "sorting": yolo_class
                        }
                        # db에 내용추가
                        insert_accident_data(accident_info)
                        # 다른 서버로 전송
                        response = sendData(accident_info)
...
# 스트리밍 비디오 처리
def process_streaming_video(source, densenet_model, yolo_model, device, gps_info):
    cap = cv2.VideoCapture(source)
    if not cap.isOpened():
        return ['Failed to open video source']
...
def upload_link():
    ...
    try:
        ...
        yolo_model = load_yolo_model('path_to_yolo_model.pt')
        gps_info = request.form.get('gps_info', '')
        if 'streaming' in request.json and request.json['streaming']:
            results = process_streaming_video(video_link, densenet_model, yolo_model, device, gps_info)
        else:
